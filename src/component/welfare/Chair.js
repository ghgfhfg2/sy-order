...
function Chair() {
  ...
  const reservation = (num, time, chair) => {
    ...
    if (ThisWeekRserv + ThisWeekPenalty >= charSetting.ableNum) {
      message.error(`예약은 일주일에 ${charSetting.ableNum}번까지 가능합니다.`);
      return;
    }
    const type =
      chair === 1
  ...
  const onCancel = (date, num, room) => {
    let limitHour = charSetting.panaltyTime || 2;
    let nowDate = new Date("2023-09-19 15:00");
    ...
    if (reservCalcMin - nowCalcMin < charSetting.panaltyTime * 60) {
      if (limitHour > 0) {
