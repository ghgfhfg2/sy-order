...
function AdminOrderList() {
  ...
  const [searchStart, setSearchStart] = useState(curDate);
  const [searchEnd, setSearchEnd] = useState(curDate);
  ...
  const [hisroty, setHisroty] = useState(1);
  const onHistory = (e) => {
    setHisroty(e.target.value);
  };
  ...
  }, [searchStart, searchEnd, hisroty, lastOrderTime]);
  ...
  const onSelectDateStart = (date, dateString) => {
    setSearchStart(getFormatDate(date._d));
    setRender(!Render);
  };

  const onSelectDateEnd = (date, dateString) => {
    setSearchEnd(getFormatDate(date._d));
    setRender(!Render);
  };

  const disabledDate = (current) => {
    return current < moment().subtract(35, "days") || current > moment();
  };
  ...
  return (
    <>
      ...
      <Radio.Group onChange={onHistory} defaultValue="1" buttonStyle="solid">
      ...
      {hisroty == 2 && (
        <>
          <div style={{ marginTop: "12px" }}>
            {searchStart && `${searchStart.full_}`}
            {searchEnd && ` ~ ${searchEnd.full_}`}
