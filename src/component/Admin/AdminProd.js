...
function AdminProd() {
  ...
  const onSubmitProd = async (values) => {
    ...
    try {
      ...
      values.sort_num >= 0
        ? (values.sort_num = values.sort_num)
        : (values.sort_num = 9999);
      await firebase
        .database()
        .ref("products")
        .child(uuid())
