...
function LunchCheck() {
  ...
  while (j < 14) {
    if (j == 6) {
      nextWeekArr.push(
        getFormatDate(new Date(date.setDate(date.getDate() + 3)))
      );
    } else {
      nextWeekArr.push(
        getFormatDate(new Date(date.setDate(date.getDate() + 1)))
      );
    }
  ...
  useEffect(() => {
    ...
    firebase
      ...
      .ref("lunch")
      .once("value", (snapshot) => {
        if (!snapshot.val() || !snapshot.val().orderTimeStart) {
          return;
        }
    ...
    firebase
      .database()
      .ref(`lunch/user/${userInfo.uid}/checkList`)
      .on("value", (snapshot) => {
        let arr = [];
        snapshot.forEach((el) => {
          arr.push({
  ...
  const onsubmit = () => {
    let list = [
      ...weekList.current.querySelectorAll("li"),
      ...weekList2.current.querySelectorAll("li"),
      ...weekList3.current.querySelectorAll("li"),
    ];
    let checkList = {};
    list.map((el) => {
    ...
    firebase.database().ref(`lunch/user/${userInfo.uid}`).update({
      checkList,
  ...
  const onTodayCheck = (date) => {
    if (limitTime >= orderTimeStart && limitTime < orderTimeEnd) {
      firebase
        .database()
        .ref(`lunch/user/${userInfo.uid}/checkList/${date}`)
        .update({
          confirm: 1,
        });
    } else {
      message.error("체크시간이 아닙니다.");
    }
  };
  ...
  return (
    <>
      <div className="item-info-box">{ItemInfo && <pre>{ItemInfo}</pre>}</div>
        ...
        {PrevState &&
          PrevState.map((el, idx) => (
            <li key={idx} data-date={el.full}>
              ...
              {!ModifyState && (
                <>
                  <div className="item-info">{`${el.item ? el.item : ""}`}</div>
                    ...
                    <>
                      ...
                      ItemList.length == l_idx + 1 ? (
                        ...
                        <Checkbox
                          ...
                          disabled
                          defaultChecked={
                            el.item && el.item.includes(list) ? true : false
                          }
                        >
                          {list}
                        </Checkbox>
        ...
        {CurState &&
          CurState.map((el, idx) => (
            <li
              key={idx}
              data-date={el.full}
              className={el.full == curDate.full ? "today" : ""}
            >
              <span className="date">
                {`${el.month}.${el.day}(${el.week})`}
              </span>
                        ...
                        <>
                          <Button
                            ...
                            disabled={
                              !el.item
                                ? true
                                : limitTime >= orderTimeStart &&
                                  limitTime < orderTimeEnd
                                ? false
                                : true
                            }
                            onClick={() => {
                              onTodayCheck(el.full);
                            }}
                          >
                            {el.item ? "식단확인" : "확인불가"}
                          </Button>
